...
class FileEntryPolicy
{
    ...
    public function index(BaseUser $user, array $entryIds = null, $userId = null)
    {
        ...
        // trying to view all file entries, but no admin or files.view permissions
        if ( ! $entryIds) {
            return false;
        }
    ...
    private function userHasPermission(BaseUser $user, $permission, $entryIds)
    {
        ...
        $entries = $user->entries(['owner' => false])
        ...
        return $entriesUserHasPermissionsFor && $entriesUserHasPermissionsFor === $allEntries;
    }
}
