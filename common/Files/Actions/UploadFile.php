...
class UploadFile
{
    ...
    private function storeUpload($diskName, FileEntry $entry, $contents, $params)
    {
        ...
        if ($diskName !== 'public') {
            try {
                $this->maybeCreateThumbnail($disk, $entry, $contents, $options['visibility']);
    ...
    private function maybeCreateThumbnail(FilesystemAdapter $disk, FileEntry $entry, $contents, string $visibility)
    {
