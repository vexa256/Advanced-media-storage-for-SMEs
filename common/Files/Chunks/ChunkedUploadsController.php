...
class ChunkedUploadsController extends BaseController
{
    ...
    public function load()
    {
        ...
        if (File::exists($chunkDir)) {
            $uploadedChunks = collect(File::files($chunkDir))
                ->map(function($path) {
            ...
            if ($uploadedChunks->count() === $totalChunks) {
                $fileEntry = app(AssembleFileFromChunks::class)->execute($this->request->except('file'));
            }
        }

        return $this->success(
            $this->transformFileEntryResponse(['uploadedChunks' => $uploadedChunks, 'fileEntry' => $fileEntry], $this->request->all())
        );
    }
