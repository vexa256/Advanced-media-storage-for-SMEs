...
class SpotifyNormalizer
{
    ...
    public function track($spotifyTrack, $albumName = null)
    {
        $track = [
            ...
            'album_name' =>  $albumName ?: $spotifyTrack['album']['name'],
            'artists' => collect(),
    ...
    public function album($spotifyAlbum, $fullyScraped = false)
    {
        $album = [
            ...
            // TODO: allow attaching multiple artists to album
            'artist' => $this->artist($spotifyAlbum['artists'][0]),
            'spotify_id' => $spotifyAlbum['id'],
        ];

        if ($tracks = Arr::get($spotifyAlbum, 'tracks')) {
            $tracks = isset($spotifyAlbum['tracks']['items']) ? $spotifyAlbum['tracks']['items'] : Arr::get($spotifyAlbum, 'tracks', []);
