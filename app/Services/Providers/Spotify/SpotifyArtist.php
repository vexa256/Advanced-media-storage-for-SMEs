...
class SpotifyArtist {
     ...
     * @return array|false
     */
    public function getArtist(Artist $artist = null, $spotifyId = null) {
        $spotifyId = $spotifyId ?: ($artist ? $artist->spotify_id : null);
        if ($spotifyId) {
            $spotifyArtist = $this->httpClient->get("artists/{$spotifyId}");
        } else {
            $spotifyArtist = $this->findByName($artist->name);
            if ($spotifyArtist) {
        ...
        $partialAlbums = $this->httpClient->get("artists/{$spotifyId}/albums?offset=0&limit=50&album_type=album,single");
        $albums = $this->getFullAlbums($partialAlbums);
        $similar = $this->getSimilar($spotifyArtist['id']);
    ...
    private function findByName($artistName)
    {
        ...
        try {
            $response = $this->httpClient->get("search?type=artist&q=$artistName&limit=50");
        ...
        if (isset($response['artists']['items'][0])) {
            foreach ($response['artists']['items'] as $spotifyArtist) {
                ...
                $normalizedSpecifiedName = str_replace([' ', '.'], '', strtolower($artistName));

                if ($normalizedSpotifyName === $normalizedSpecifiedName) {
                    $artist = $spotifyArtist; break;
                }

                if (Str::contains($normalizedSpotifyName, $normalizedSpecifiedName)) {
    ...
    private function makeAlbumsIdString($response) {
        ...
        if (isset($response['items']) && count($response['items'])) {
            foreach($response['items'] as $album) {
                ...
                if (Str::contains($name, '(clean')) continue;
